...
If a later session starts before an earlier session ends, then the **earlier session card** is
...
Schedule data may come from a single backend system, or in some setups, data from multiple backends
is **merged** into a single file.
...
Another scenario to consider is when the backend system supports a session being **scheduled multiple
times**. This saves the organizer from having to create multiple sessions for the same content.
These sessions are given the **same ID**. However, the app will not be able to display these
sessions correctly for the same reasons as described above.
