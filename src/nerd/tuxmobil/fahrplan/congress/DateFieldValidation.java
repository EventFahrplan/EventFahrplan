...
public class DateFieldValidation {
	...
	/**
	 * Returns true if the time stamps in the "date" fields of each "event" are within a valid time range.
	 * The time range is defined by the "date" attribute of the "day" fields. Otherwise false is returned.
	 */
	public boolean validate() {
		SQLiteDatabase db = mLecturesDatabase.getReadableDatabase();
		Cursor lectureCursor = null;
		try {
			// Order by "date" column relying on that the date formatted in reverse order: yyyy-MM-dd'T'HH:mm:ssZ
			lectureCursor= db.query(LecturesTable.NAME, null, null, null, null, null, Columns.DATE);

			// Prepare time range (first day)
			lectureCursor.moveToFirst();
			String firstDateString = lectureCursor.getString(lectureCursor.getColumnIndex(LecturesTable.Columns.DATE));
			...
			String lastDateString = lectureCursor.getString(lectureCursor.getColumnIndex(LecturesTable.Columns.DATE));
			...
			while (!lectureCursor.isAfterLast()) {
				long dateUtcInMilliseconds = lectureCursor.getLong(lectureCursor.getColumnIndex(LecturesTable.Columns.DATE_UTC));
				...
				if (!DateHelper.dateIsWithinRange(dateUtc, dateRange)) {
					String eventId = lectureCursor.getString(lectureCursor.getColumnIndex(LecturesTable.Columns.EVENT_ID));
					String formattedDateUtc = DateHelper.getFormattedDate(dateUtc);
					String errorMessage = "Field <date> " + formattedDateUtc + " of event " + eventId + 
